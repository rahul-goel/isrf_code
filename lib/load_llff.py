...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, load_depths=False, args=None):
    ...
    if args is not None and args.distill_active:
        fts = load_features(file=os.path.join(basedir, "features.pt"), imhw=(height, width))
        #fns = [x.split('/')[-1] for x in imgfiles]
        #fts = torch.zeros([height, width, 64, imgs.shape[-1]])
        #for i,fn in enumerate(i,fns):
        #    fts[:,:,:,i] = fts_dict[fn].permute(1, 2, 0)
        #fts = torch.stack([fts_dict[fn].permute(1,2,0) for fn in fns], dim=-1)
    ...
    return poses, bds, imgs, depths, fts
...
def load_llff_data(basedir, factor=8, width=None, height=None,
    ...
    poses, bds, imgs, fts = _load_data(basedir, factor=factor, width=width, height=height,
    ...
    if args is not None and args.distill_active:
        fts = torch.moveaxis(fts, -1, 0).float()
    images = imgs
...
#             zloc = np.percentile(tt, 10, 0)[2]
    ...
    return images, depths, poses, bds, render_poses, i_test, fts
