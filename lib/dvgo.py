...
class DirectVoxGO(torch.nn.Module):
    ...
    def voxel_count_views(self, rays_o_tr, rays_d_tr, imsz, near, far, stepsize, downrate=1, irregular_shape=False, distill_active=False):
        ...
        if distill_active:
            tmp = grid.create_grid(
                   self.k0_type, channels=1, world_size=self.f_world_size,
                    xyz_min=self.xyz_min, xyz_max=self.xyz_max,
                    config=self.density_config)
            f_count = torch.zeros_like(tmp.get_dense_grid())

        device = rng.device
        for rays_o_, rays_d_ in zip(rays_o_tr.split(imsz), rays_d_tr.split(imsz)):
            ones = grid.DenseGrid(1, self.world_size, self.xyz_min, self.xyz_max)
            if distill_active:
                f_ones = grid.DenseGrid(1, self.f_world_size, self.xyz_min, self.xyz_max)
            if irregular_shape:
            ...
            for rays_o, rays_d in zip(rays_o_, rays_d_):
                ...
                if distill_active:
                    f_ones(rays_pts).sum().backward()
            with torch.no_grad():
                count += (ones.grid.grad > 1)
                if distill_active:
                    f_count += (f_ones.grid.grad > 1)
...
@torch.no_grad()
def get_training_rays_flatten(rgb_tr_ori, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y, f_tr_ori=None):
    print('get_training_rays_flatten: start')
    assert len(rgb_tr_ori) == len(train_poses) and len(rgb_tr_ori) == len(Ks) and len(rgb_tr_ori) == len(HW)
    if f_tr_ori is not None:
        assert len(f_tr_ori) == len(train_poses) and len(f_tr_ori) == len(Ks) and len(f_tr_ori) == len(HW)
    eps_time = time.time()
    DEVICE = rgb_tr_ori[0].device
    N = sum(im.shape[0] * im.shape[1] for im in rgb_tr_ori)
    rgb_tr = torch.zeros([N,3], device=DEVICE)
    f_tr = torch.zeros([N,64], device=DEVICE)
    ...
    if f_tr_ori is None:
        ...
        for c2w, img, f_img, (H, W), K in zip(train_poses, rgb_tr_ori, f_tr_ori, HW, Ks):
            assert img.shape[:2] == (H, W)
            rays_o, rays_d, viewdirs = get_rays_of_a_view(
                    H=H, W=W, K=K, c2w=c2w, ndc=ndc,
                    inverse_y=inverse_y, flip_x=flip_x, flip_y=flip_y)
            n = H * W
            rgb_tr[top:top+n].copy_(img.flatten(0,1))
            f_tr[top:top+n].copy_(f_img.flatten(0,1))
            rays_o_tr[top:top+n].copy_(rays_o.flatten(0,1).to(DEVICE))
            rays_d_tr[top:top+n].copy_(rays_d.flatten(0,1).to(DEVICE))
            viewdirs_tr[top:top+n].copy_(viewdirs.flatten(0,1).to(DEVICE))
            imsz.append(n)
    ...
    if f_tr_ori is None:
        ...
        return rgb_tr, f_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz


@torch.no_grad()
def get_training_rays_in_maskcache_sampling(rgb_tr_ori, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y, model, render_kwargs, f_tr_ori=None):
