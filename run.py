...
def load_model(args, cfg, data_dict, device):
    ...
    return render_viewpoints_kwargs, None, start

@torch.no_grad()
def render_single_image(rvk, cfg, data_dict, idx, want_features, render_fct):
    rvk['model'] = rvk['model'].cuda()
    rgbs, _, _, features = render_viewpoints(
            render_poses=data_dict['poses'][idx:idx+1],
            HW=data_dict['HW'][idx:idx+1],
            Ks=data_dict['Ks'][idx:idx+1],
            distill_active=want_features,
            cfg=cfg,
...
@torch.no_grad()
def render_images(render_viewpoints_kwargs: dict, cfg, data_dict: dict, imageset: str, want_features: bool, render_fct):
    i_type = "i_test"
    if imageset == "test":
        i_type = "i_test"
    elif imageset == "train":
    ...
    render_viewpoints_kwargs['model'] = render_viewpoints_kwargs['model'].cuda()
    rgbs, _, _, features = render_viewpoints(
            render_poses=data_dict['poses'][data_dict[i_type]],
            HW=data_dict['HW'][data_dict[i_type]],
            Ks=data_dict['Ks'][data_dict[i_type]],
            distill_active=want_features,
            cfg=cfg,
            render_fct=render_fct,
            **render_viewpoints_kwargs)
...
def do_kmeans_clustering(mask_index, feature_image):
    ...
    fimg = feature_image.permute(2, 0, 1)

    fimg = torch.nn.functional.normalize(fimg, dim=0)
    fimg = torch.nn.functional.interpolate(fimg.unsqueeze(0), size=(height, width), mode="nearest").squeeze(0)
    features = fimg[:, mask_index]
    faiss_kmeans = kmeans(features)
    return faiss_kmeans

@torch.no_grad()
def do_kmeans_clustering_multiview(indices, fimages):
    # normalize before clustering
    height, width = indices[0].shape
    all_features = []
    for i, index in enumerate(indices):
        if index.sum():
            fimg = torch.tensor(fimages[i]).permute(2, 0, 1)
            fimg = torch.nn.functional.normalize(fimg, dim=0)
            features = fimg[:, index]
